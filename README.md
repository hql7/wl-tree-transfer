...
因为公司业务使用 vue 框架，ui 库使用的 element-ui。在市面上找到一个好用的 vue 树形穿梭框组件都很难，又不想仅仅因为一个穿梭框在 element-ui 之外引入其他重量级插件，因此就有了 wl-tree-transfer。轻量，易用，无需投入其他学习成本。
...
> 然后你可以像使用普通组件一样使用 wl-tree-transfer
...
```js
    <template>
      <div>
        ...
        ...
        // 使用树形穿梭框组件
        <wl-tree-transfer :title="title" :from_data='fromData' :to_data='toData' :defaultProps="{label:'label'}" @add-btn='add' @remove-btn='remove' :mode='mode' height='540px' filter openAll>
      </wl-tree-transfer>
      </div>
    </template>
